...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        await safe_reply(update, "⏰ 任務執行逾時，請檢查背景狀態。")
...
async def team_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not user_task:
        await safe_reply(update, "請在指令後加上需求！例如：/team 幫我將網頁背景改為深色，並修改按鈕樣式")
        return
    ...
    # 注入 PATH 與自動批准模式，確保 git push 不會卡住
    env = os.environ.copy()
    env["PATH"] = "/opt/homebrew/bin:/usr/local/bin:/usr/bin:/bin:/usr/sbin:/sbin:" + env.get("PATH", "")
    env["GOOSE_MODE"] = "auto"
    ...
    await update.message.reply_text("🚀 Web 開發團隊已啟動！正在進行：規劃 -> 實作 -> 測試 -> 部署，這可能需要幾分鐘...")
    ...
    try:
        process = subprocess.run(
            ...
            text=True,
            env=env,
            timeout=300 # 5分鐘，因為要執行 Git 和多步修改
        )
        ...
        await safe_reply(update, "⏰ 團隊執行逾時！請檢查專案資料夾看進度，或者檔案可能已經推上去了。")
    except Exception as e:
        await safe_reply(update, f"❌ 錯誤: {str(e)}")
...
async def clear_memory(update: Update, context: ContextTypes.DEFAULT_TYPE):
    global chat_history
    chat_history = []
    await update.message.reply_text("🧹 記憶已清除！現在是一隻全新失憶的鵝了。")
...
if __name__ == '__main__':
    if not get_goose_path():
        ...
        print("🚀 Telegram Goose Bot (Vercel 自動部署版) 已啟動...")
        app.run_polling()
